...
void generate_alphabet(uint8_t* input) {
	int i=0;
...
void hash_md5(uint8_t* input, uint8_t* result)
{
    // benchmark
...
void guess(uint8_t* prefix, int level, uint8_t* input, int max_depth, uint8_t* alphabet)
{
...
//		{
//			printf("Input string found: %s", tmp);
//		}

		if(level < max_depth)
		{
			guess(tmp, level, input, max_depth, alphabet);
		}
	}
...
int main(int argc, char **argv) {
	...
	guess("", 0, msg, len, alphabet);


//	hash_md5(input, current_hash);
//	show_hash(current_hash);
